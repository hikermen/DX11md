	...
	namespace d3{
		...
		namespace shader{
			namespace detail{
				...
				std::shared_ptr<ID3D11HullShader> CreateHullShader(ID3D11Device* device, BYTE* shader, size_t size)
				{
					return detail::CreateShader(device, shader, size, &ID3D11Device::CreateHullShader);
				}
				std::shared_ptr<ID3D11DomainShader> CreateDomainShader(ID3D11Device* device, BYTE* shader, size_t size)
				{
					return detail::CreateShader(device, shader, size, &ID3D11Device::CreateDomainShader);
				}
			...
			std::shared_ptr<ID3D11HullShader> CreateHullShader(ID3D11Device* device, ID3DBlob * blob)
			{
				return detail::CreateHullShader(device, (BYTE*)blob->GetBufferPointer(), blob->GetBufferSize());
			}
			std::shared_ptr<ID3D11DomainShader> CreateDomainShader(ID3D11Device* device, ID3DBlob * blob)
			{
				return detail::CreateDomainShader(device, (BYTE*)blob->GetBufferPointer(), blob->GetBufferSize());
			}
			std::shared_ptr<ID3D11InputLayout> CreateInputLayout(
